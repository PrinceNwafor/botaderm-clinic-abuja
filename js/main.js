/* ==========================================================================
   BOTADERM AESTHETIC CLINIC — MAIN JAVASCRIPT
   ========================================================================== */
...
(function () {
  ...
  /* --------------------------------------------------------------------------
     0. BOTADERM CONFIG — edit these in ONE place
     -------------------------------------------------------------------------- */
  // International format, digits only (Nigeria = 234). Placeholder numbers — replace with real clinic lines.
  const BOTADERM = {
    whatsapp:      '2348000000000',   // 👈 replace with the clinic WhatsApp number
    whatsappAbuja: '2348000000000',   // 👈 Abuja branch line
    whatsappBenin: '2348000000001',   // 👈 Benin City branch line
  };
  window.BOTADERM = BOTADERM;
  ...
  // Wire any element with [data-wa] to open WhatsApp with a prefilled message.
  // data-wa = number (optional, defaults to main line); data-wa-msg = message.
  document.querySelectorAll('[data-wa]').forEach((el) => {
    const num = el.getAttribute('data-wa') || BOTADERM.whatsapp;
    const msg = el.getAttribute('data-wa-msg') || 'Hello Botaderm, I would like to book a consultation.';
    el.setAttribute('href', waLink(num, msg));
    el.setAttribute('target', '_blank');
    el.setAttribute('rel', 'noopener');
  });

  /* Keep the fixed top bars perfectly stacked under the PREVIEW ribbon,
     even when the ribbon text wraps to 2–3 lines on small screens. */
  const ribbon = document.querySelector('.demo-ribbon');
  function syncRibbonHeight() {
    const h = ribbon ? ribbon.offsetHeight : 0;
    document.documentElement.style.setProperty('--ribbon-height', h + 'px');
  }
  if (ribbon) {
    syncRibbonHeight();
    window.addEventListener('resize', syncRibbonHeight, { passive: true });
    window.addEventListener('load', syncRibbonHeight);
  } else {
  ...
  function showToast(message, type = 'success') {
    ...
    Object.assign(toast.style, {
      ...
      background: type === 'success' ? '#1B2A4D' : '#D64550',
  ...
  function initBooking(root) {
    ...
    let current = 0;
    const state = { service: null, branch: null, date: null, time: null,
    ...
    function renderSummary() {
      ...
      // Build a WhatsApp confirmation link from the chosen details, routed to the branch line.
      const waBtn = root.querySelector('[data-wa-confirm]');
      if (waBtn) {
        const num = /benin/i.test(state.branch || '') ? BOTADERM.whatsappBenin : BOTADERM.whatsappAbuja;
        const msg =
          'Hello Botaderm 👋, I would like to confirm a consultation request:\n' +
          ...
          (state.notes ? '\n• Notes: ' + state.notes : '');
        waBtn.setAttribute('href', waLink(num, msg));
